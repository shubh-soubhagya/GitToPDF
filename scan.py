...
def scan_directory(base_dir):
    ...
    for subdir in base_path.iterdir():
        if subdir.is_dir():
            ...
            if subdir.name == '.git':
                print(f"Skipping .git directory: {subdir}")
                continue
            ...
            # Look for README.md files (skip .git files)
            readme_files = []
            for pattern in ['README.md', 'readme.md', 'Readme.md']:
                for file in subdir.glob(pattern):
                    if '.git' not in str(file):  # Skip files in .git directories
            ...
            # Look for .java files (skip .git files)
            java_files = []
            for java_file in subdir.glob('*.java'):
                if '.git' not in str(java_file):  # Skip files in .git directories
