...
def refine_problem_statement(readme_content):
    ...
    prompt = f"""Analyze the following LeetCode problem description. Extract the complete problem statement in a clean, plain text format.

Instructions for Extraction:

Preserve all technical content: Keep the problem description, examples, constraints, and requirements.

Convert HTML entities: Replace &quot; with ", &lt; with <, &gt; with >, etc.

Remove all formatting: Eliminate markdown syntax (headers, bold, italics, code blocks) and HTML tags.

Maintain structure: Keep the original sections for examples and constraints clearly separated using simple plain text dividers.

Keep examples intact: Preserve all test cases, but remove any markdown or HTML surrounding them.

Provide only the output: Do not add any commentary, analysis, or extra text. The output should be the raw, cleaned problem statement.
